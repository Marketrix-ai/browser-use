...
class BrowserSession(BaseModel):
	...
	async def save_storage_state(self, path: Path | None = None) -> None:
		...
		if (path and not path_is_storage_state) or self.browser_profile.cookies_file:
			try:
				cookies_file_path = Path(path or self.browser_profile.cookies_file or "").expanduser().resolve()
	...
	@staticmethod
	async def _get_unique_filename(directory: Path | str, filename: str) -> str:
